(function (global, factory) {
  ...
  var ImmyBox = (function () {
    function ImmyBox(element, options) {
      ...
      babelHelpers.classCallCheck(this, ImmyBox);
      ...
      })].concat(babelHelpers.toConsumableArray(this.choices.filter(function (_ref2) {
      ...
      if (this.options.showArrow) addClass(this.element, plugin_name + '_witharrow');
      ...
      assignEvent('click', function (event) {
        var node = nodeOrParentMatchingSelector(event.target, 'li.' + plugin_name + '_choice');
        if (node) {
      ...
      assignEvent('mouseenter', function (event) {
        ...
        if (node) {
          ...
          [].concat(babelHelpers.toConsumableArray(_this.queryResultArea.querySelectorAll('li.' + plugin_name + '_choice.active'))).forEach(function (li) {
            return li !== node && removeClass(li, 'active');
          });
        }
      }, this.queryResultArea, listeners);
    ...
    babelHelpers.createClass(ImmyBox, [{
      key: 'doQuery',
      ...
      value: function insertFilteredChoiceElements(query) {
        ...
        if (results.length) {
          if (this.valueFromElement(results[0]) === this.options.defaultSelectedValue) !selected_one && addClass(results[0], 'active');
        } else {
      ...
      value: function setChoices(newChoices) {
        ...
        })].concat(babelHelpers.toConsumableArray(this.choices.filter(function (_ref7) {
